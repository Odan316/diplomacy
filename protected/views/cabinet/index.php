    ...
    <div class="cabinet_left block2">
        ...
        <div class="games_list">
            <?php foreach ($master_games as $game) { ?>
                <p class="game_select"
                   data-id="<?= CHtml::encode( $game->id ) ?>"><?= CHtml::encode( $game->title ) . "(" . CHtml::encode( $game->tag ) . ")" ?></p>
            <?php } ?>
        </div>
...
<div class="cabinet_game_creation block2 popup">
    ...
    $this->widget( 'bootstrap.widgets.TbButton', [
        'label'       => 'Создать',
        'htmlOptions' => [ 'id' => 'action_game_create' ]
    ] );
    $this->widget( 'bootstrap.widgets.TbButton', [
        'label'       => 'Отмена',
        'htmlOptions' => [ 'class' => 'popup_close' ]
    ] );
    ?>
</div>